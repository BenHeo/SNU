...
tree.buydata = rpart(as.factor(RESPOND)~., data = train, control = rpart.control(cp = 0.005)) # Complexity Parameter
tree.buydata
...
boost.fit = xgboost(data = X_train, label = y_train, max.depth = 2, 
                    eta = 0.1, nround = 2, objective = "binary:logistic") # eta = stepsize = ((learning rate))
...
# verbose = 0 means don't print train-error for each iter
pred = predict(boost.fit, X_test)
mean(y_test != round(pred))
...
cc_data = cc_data[sample(1:nrow(cc_data), nrow(cc_data), replace = F),] # shuffle
