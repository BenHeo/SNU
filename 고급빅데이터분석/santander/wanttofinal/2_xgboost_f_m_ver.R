...
product.list <- colnames(fread("cleansed_train.csv", select=20:43, nrows=0))[-c(1,2,10,11)]

for(product in product.list) {
  
  print(product)
  
  if(product == "ind_cno_fin_ult1") { 
    train.date <- 6
    data.1 <- fread(paste0("train_", train.date, ".csv"), colClasses=c(indrel_1mes="character"))
    data.2 <- fread(paste0("count_", train.date, ".csv"))
    data.train1 <- cbind(data.1, data.2)
    
    train.date <- 12
    data.1 <- fread(paste0("train_", train.date, ".csv"), colClasses=c(indrel_1mes="character"))
    data.2 <- fread(paste0("count_", train.date, ".csv"))
    data.train2 <- cbind(data.1, data.2)
    data.train <- rbind(data.train1, data.train2)
    
    rm(data.train1)
    rm(data.train2)
    
  } else if(product == "ind_pres_fin_ult1") {
    train.date <- 6
    data.1 <- fread(paste0("train_", train.date, ".csv"), colClasses=c(indrel_1mes="character"))
    data.2 <- fread(paste0("count_", train.date, ".csv"))
    ...
    data.1 <- fread(paste0("train_", train.date, ".csv"), colClasses=c(indrel_1mes="character"))
    data.2 <- fread(paste0("count_", train.date, ".csv"))
    data.train3 <- cbind(data.1, data.2)
    data.train <- rbind(data.train1, data.train2, data.train3)
  ...
  data.1 <- fread("train_17.csv", colClasses=c(indrel_1mes="character"))
  data.2 <- fread("count_17.csv")
  data.val <- cbind(data.1, data.2)
  
  data.1 <- fread("test_18.csv", colClasses=c(indrel_1mes="character"))
  data.2 <- fread("count_18.csv")
  ...
  data.train <- data.train[data.train[[paste0(product,"_last")]] == 0] # 가장 최근 보유 여부가 없는 행만 사용
  ...
  data.train[, ind_actividad_cliente_last:=NULL] # from_to로 만들었으니 제거해준다
  data.val[, ind_actividad_cliente_last:=NULL]
  data.test[, ind_actividad_cliente_last:=NULL]
  
  data.train[, tiprel_1mes_last:=NULL] # from_to로 만들었으니 제거해준다
  data.val[, tiprel_1mes_last:=NULL]
  data.test[, tiprel_1mes_last:=NULL]
  
  data.train$n_products_last <- apply(data.train[, (1:20)+20+20, with=FALSE], 1, sum) # total products
  ...
  exp.var <- colnames(data.test)[-(1:3)]
  for(var in exp.var) {
