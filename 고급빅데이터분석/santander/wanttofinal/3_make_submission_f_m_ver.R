...
mode <- "submission"
product.list <- colnames(fread("cleansed_train.csv", select=20:43, nrows=0))
date.list <- 17
...
for(i in 1:length(date.list)) {
  ...
  # normalize
  pred.sum <- result.temp[product %in% product.list[-c(3,17,18)], .(sum=sum(pr)), by=ncodpers]
  result.temp <- merge(result.temp, pred.sum, by="ncodpers")
  result.temp <- result.temp[, .(ncodpers, log_pr=log(pr/sum), product)] # 특정 고객이 특정 상품을 살 log 확률이 나온다
